...
public class Water : MonoBehaviour
{
	...
	ref float[,] GetPreviousBuffer()
	{
		return ref ((frame % 2 == 0) ? ref m_buffer1 : ref m_buffer2);
	}

	ref float[,] GetCurrentBuffer()
	{
	...
	void Update()
	{
		...
		{
			float[,] previous = GetPreviousBuffer();
			float[,] current = GetCurrentBuffer();
		...
		for (int x = 0; x < m_xMeshVertexNum; x++)
		{
			for (int z = 0; z < m_zMeshVertexNum; z++)
			{
				...
				float height = current[x, z];
				m_vertices[x + (z * m_xMeshVertexNum)].y = height;
			}
		}
